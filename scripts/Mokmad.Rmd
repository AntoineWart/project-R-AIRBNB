...
Pour chaque coef il faut le multiplier par 100 et on obtiens l'influence de critère sur le prix.
Exemple: NeighbourHood = Brooklyn donne le coef 0.23 donc si le critère est valider pour le airbnb en question alors on prends le prix de base (118.90$) et on rajoute
23% de cette valeur.
```{r}
coefs <- coefficients(lm_fit)
effects_price <- expm1(coefs)
round(effects_price, 2)
```
